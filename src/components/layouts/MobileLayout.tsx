...
import MainLayoutPage, { LayoutConfigProps, UnknownProps } from './MainLayout'
...
export const withMobileLayoutPage = <T extends UnknownProps>(
  ...
  layoutProps?: MobileLayoutConfigProps | ((pageProps: T) => MobileLayoutConfigProps)
) => {
  const MobileLayoutPage: FunctionComponent<T> = (pageProps) => {
    ...
    const { classNameMobile } = layoutPropsWithPageProps

    return (
      <MainLayoutPage {...layoutPropsWithPageProps}>
        <div className={clsx(['relative max-w-[500px] mx-auto w-full flex flex-col min-h-screen shadow-xl', classNameMobile])}>
