...
const Image: FunctionComponent<Props> = (props) => {
  const { src, effect, size, style, className, wrapperClassName, placeholderSrc, onError, ...lazyloadProps } = props

  const imgSrc = useMemo(() => (src as any)?.src ?? src ?? placeholderSrc ?? DEFAULT_PLACEHOLDER, [src, placeholderSrc])
