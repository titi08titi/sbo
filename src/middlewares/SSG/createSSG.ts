...
import { GetStaticPathsContext, GetStaticPathsResult, type GetStaticPropsContext, type GetStaticPropsResult } from 'next'
...
const createSSGPaths =
  <T extends ParsedUrlQuery = ParsedUrlQuery>(next: SSGPathsNextFunction<T>) =>
  (ctx: SSGPathsContext): SSGPathsResult<T> =>
    next(ctx)
...
const createSSGProps =
  <T>(next: SSGPropsNextFunction<T>) =>
  (ctx: SSGPropsContext): SSGPropsResult<T> =>
    next(ctx)
